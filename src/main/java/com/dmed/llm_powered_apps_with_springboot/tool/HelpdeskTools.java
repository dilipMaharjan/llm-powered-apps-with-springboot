...
public class HelpdeskTools {
    ...
    public String getAllTicketsSummary() {
        ...
        String result = "Helpdesk System Summary:\n" +
                String.format("Total Tickets: %d\n\n", tickets.size()) +
                "Status Breakdown:\n" +
                String.format("- Open: %d\n", openCount) +
                String.format("- In Progress: %d\n", inProgressCount) +
                String.format("- Resolved: %d\n", resolvedCount) +
                String.format("- Closed: %d\n", closedCount) +
                "\nPriority Alerts:\n" +
                String.format("- Critical: %d\n", criticalCount) +
                String.format("- High: %d\n", highCount);

        return result;
    }

    /**
     * Tool to list all helpdesk tickets
     */
    @Tool(description = "List all helpdesk tickets in the system with their details")
    public String listAllTickets() {
        log.info("AI Tool: Listing all tickets");
        ...
        StringBuilder result = new StringBuilder();
        result.append(String.format("All Helpdesk Tickets (%d total):\n\n", tickets.size()));

        for (HelpdeskTicket ticket : tickets) {
            result.append(String.format("Ticket #%d: %s\n", ticket.getId(), ticket.getTitle()));
            ...
            result.append(String.format("  Assigned to: %s | Reported by: %s\n", ticket.getAssignedTo(), ticket.getReportedBy()));
            result.append(String.format("  Created: %s | Updated: %s\n\n", ticket.getCreatedAt(), ticket.getUpdatedAt()));
        }
